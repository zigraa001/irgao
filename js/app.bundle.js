...
// Service code -> human label (used for display + persistence).
const SERVICE_LABELS = { taxi: 'Air Taxi', golden: 'Golden Hour', shuttle: 'Air Shuttle', drones: 'Drone Delivery', 'drone-rental': 'Drone Rental' };
...
const demoLocations = {
  ...
  'Taramani Tidel Park Vertiport':[12.9897, 80.2478],
  'Adyar Vertiport, Chennai':     [13.0063, 80.2574],
  'Sholinganallur Vertiport':     [12.9010, 80.2279],
  'T. Nagar Vertiport, Chennai':  [13.0418, 80.2341],
  'Anna Nagar Vertiport, Chennai':[13.0846, 80.2101],
  'Porur Vertiport, Chennai':     [13.0382, 80.1565],
  'Ambattur Vertiport, Chennai':  [13.1143, 80.1548],
  'Guindy Vertiport, Chennai':    [13.0102, 80.2120],
  'Mylapore Vertiport, Chennai':  [13.0339, 80.2694],
  'ECR Injambakkam Vertiport':    [12.9209, 80.2513],
  'Fortis Malar Hospital, Adyar': [13.0056, 80.2564],
  'Gleneagles Hospital, Chennai': [12.9299, 80.2444],

  // Tamil Nadu — cities & towns within eVTOL range of Chennai
  'Mahabalipuram Vertiport':      [12.6208, 80.1945],
  'Chengalpattu Vertiport':       [12.6920, 79.9760],
  'Kanchipuram Vertiport':        [12.8342, 79.7036],
  'Sriperumbudur Vertiport':      [12.9700, 79.9450],
  'Tiruvallur Vertiport':         [13.1439, 79.9094],
  'Puducherry Vertiport':         [11.9416, 79.8083],
  'Cuddalore Vertiport':          [11.7480, 79.7714],
  'Vellore Vertiport':            [12.9165, 79.1325],
  'CMC Hospital, Vellore':        [12.9249, 79.1353],
  'Tiruvannamalai Vertiport':     [12.2253, 79.0747],
  'Hosur Vertiport':              [12.7409, 77.8253],
  'Salem Vertiport':              [11.6643, 78.1460],
  'Thanjavur Vertiport':          [10.7870, 79.1378],
  'Tiruchirappalli Vertiport':    [10.7905, 78.7047],
  'Erode Vertiport':              [11.3410, 77.7172],
  'Coimbatore Vertiport':         [11.0168, 76.9558],
  'Madurai Vertiport':            [9.9252, 78.1198],
...
function adminSectionPage(name) {
  if (name === 'taxi-live' || name === 'drone-live' || name === 'live') return 'live';
  if (name === 'taxi-bookings' || name === 'drone-bookings' || name === 'bookings') return 'bookings';
  return name;
}

function showAdminSection(name) {
  if (name === 'taxi-live') adminLiveKind = 'taxi';
  else if (name === 'drone-live') adminLiveKind = 'drone';
  else if (name === 'taxi-bookings') adminBookingsKind = 'taxi';
  else if (name === 'drone-bookings') adminBookingsKind = 'drone';
  else if (name === 'live') name = adminLiveKind === 'drone' ? 'drone-live' : 'taxi-live';
  else if (name === 'bookings') name = adminBookingsKind === 'drone' ? 'drone-bookings' : 'taxi-bookings';

  var page = adminSectionPage(name);
  const section = document.getElementById('admin-section-' + page);
  if (!section) return;
  ...
  if (page !== 'live' && adminLivePollInterval) {
    clearInterval(adminLivePollInterval);
    adminLivePollInterval = null;
  }
  if (page !== 'live') stopAdminDroneLive();
...
function adminDashboardHtml(stats) {
  ...
  var primaryRow =
    '<div class="adm-grid adm-grid--spaced">' +
      ...
      '<div class="adm-span-3 adm-kpi-link" onclick="showAdminSection(\'taxi-bookings\')">' + admKpi(ADM_ICONS.bookings, 'navy', (t.totalBookings || 0).toLocaleString('en-IN'), 'Total bookings') + '</div>' +
      '<div class="adm-span-3 adm-kpi-link" onclick="showAdminSection(\'' + ((t.taxiLive || 0) >= (t.droneLive || 0) ? 'taxi-live' : 'drone-live') + '\')">' + admKpi(ADM_ICONS.plane, 'amber', (t.live || 0).toLocaleString('en-IN'), 'Live trips', '<div class="adm-kpi-chip adm-kpi-chip--amber">Live</div>') + '</div>' +
      ...
      '<div class="adm-live-hero">' +
        ...
        '<div class="adm-live-hero-label">' +
          ((t.taxiLive || 0) || (t.droneLive || 0)
            ...
            : 'trips in progress') +
        '</div>' +
        '<div class="adm-live-btn-row">' +
          '<button type="button" class="adm-live-btn" onclick="showAdminSection(\'taxi-live\')">Air taxi map</button>' +
          '<button type="button" class="adm-live-btn" onclick="showAdminSection(\'drone-live\')">Drone map</button>' +
        '</div>' +
      '</div>' +
    '</div></div>';
...
function openAdminBookings(filter) {
  ...
  showAdminSection('taxi-bookings');
}

function openAdminDroneBookings() {
  adminBookingsFilter = 'all';
  adminBookingsOffset = 0;
  showAdminSection('drone-bookings');
}

function switchAdminBookingsKind(kind) {
  showAdminSection(kind === 'drone' ? 'drone-bookings' : 'taxi-bookings');
}
...
function syncAdminBookingsKindUI() {
  ...
  if (adminBookingsKind === 'drone') {
    ...
    if (sub) sub.textContent = 'Operator-sent drops, tracking keys, and live deliveries.';
    if (inflightBtn) inflightBtn.textContent = 'Live';
    var send = document.getElementById('admin-drone-send');
    if (send) {
      send.hidden = false;
      fillAdminDroneSendPoints();
    }
  } else {
    if (title) title.textContent = 'Air Taxi Bookings';
    if (sub) sub.textContent = 'Previous trips, in-flight rides, and completed bookings.';
    if (inflightBtn) inflightBtn.textContent = 'In flight';
    var sendOff = document.getElementById('admin-drone-send');
    if (sendOff) sendOff.hidden = true;
  }
}

function switchAdminLiveKind(kind) {
  showAdminSection(kind === 'drone' ? 'drone-live' : 'taxi-live');
}
...
function renderAdminDroneBookingsTable(bookings) {
  ...
  var rows = bookings.map(function (b) {
    ...
    var key = b.trackingKey || '';
    return '<tr>' +
      '<td><strong>#' + b.id + '</strong></td>' +
      '<td><div>' + escapeHtml(b.customerName || b.recipientEmail || 'Unknown') + '</div><div class="admin-users-meta">' + escapeHtml(b.customerEmail || b.recipientEmail || '') + '</div></td>' +
      '<td>' + route + '</td>' +
      '<td>' + escapeHtml(b.parcelType || b.serviceName || '—') + '</td>' +
      '<td>' + statusBadgeHtml(b.status) + '</td>' +
      '<td>' + (key ? '<code>' + escapeHtml(key) + '</code>' : '—') + '</td>' +
      '<td>' + escapeHtml(b.droneCallsign || b.operatorName || '—') + '</td>' +
      '<td>' + formatAdminBookingWhen(b.createdAt) +
        (live ? '<div class="admin-users-meta"><button type="button" class="admin-btn-sm" onclick="showAdminSection(\'drone-live\')">Live map</button></div>' : '') +
        (key ? '<div class="admin-users-meta"><button type="button" class="admin-btn-sm" onclick="resendAdminDroneTrack(' + b.id + ')">Resend track email</button></div>' : '') +
      '</td>' +
    '</tr>';
  }).join('');
  list.innerHTML =
    '<table class="admin-table">' +
      '<thead><tr>' +
        '<th>ID</th><th>Customer</th><th>Route</th><th>Parcel</th><th>Status</th><th>Track key</th><th>Drone</th><th>Booked</th>' +
      '</tr></thead>' +
      '<tbody>' + rows + '</tbody>' +
    '</table>';
}
...
function fillAdminDroneSendPoints() {
  const fromSel = document.getElementById('admin-drone-send-from');
...
async function submitAdminDroneSend() {
  const err = document.getElementById('admin-drone-send-error');
  const btn = document.getElementById('admin-drone-send-btn');
  if (err) err.textContent = '';
  const email = ((document.getElementById('admin-drone-send-email') || {}).value || '').trim();
  const fromName = (document.getElementById('admin-drone-send-from') || {}).value;
  const toName = (document.getElementById('admin-drone-send-to') || {}).value;
  if (!email) {
  ...
  try {
    const res = await apiFetch('/api/drones/admin/send', {
      method: 'POST',
      headers: AUTH.headers(),
      body: JSON.stringify({
        ...
        pickupName: fromName,
        dropName: toName,
        parcelType: (document.getElementById('admin-drone-send-parcel') || {}).value,
      }),
    });
    const data = await res.json();
    ...
    showToast((data.emailed ? 'Tracking email sent · ' : 'Drop created · key ') + (key || email), 'success');
    const emailEl = document.getElementById('admin-drone-send-email');
    ...
    if (btn) {
      btn.disabled = false;
      btn.textContent = 'Send & email tracking key';
    }
  }
}

async function resendAdminDroneTrack(id) {
  try {
    const res = await apiFetch('/api/drones/admin/bookings/' + id + '/resend-track', {
      method: 'POST',
      headers: AUTH.headers(),
    });
    const data = await res.json();
...
const CAMPUS_POINTS = {
  ...
  'Pine Mess': [31.78079, 76.99665],
  'Oak Mess': [31.78182, 76.99885],
  'Drongo Canteen': [31.78095, 76.99671],
  'Village Square': [31.78108, 76.99452],
  'C V Raman Guest House': [31.78142, 76.99437],
  'Auditorium Complex': [31.78105, 76.99399],
  'Academic Block': [31.78031, 76.99631],
